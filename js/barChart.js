var barChart = function () {
    ...
    let titleText;
    let barRectFill = 'steelblue';
    let xScaleDomain;
    let xValue;
    let yValue;
    let displayMode = 'bar';
    let zeroBasedYAxis = false;
    let tooltipText = (d) => {
      let str = new String();
      let counter = 0;
    ...
    function drawChart() {
        if (chartDiv) {
            ...
            let yScale = d3.scaleLinear()
                .domain(zeroBasedYAxis ? [0, d3.max(chartData, d => yValue(d))] : d3.extent(chartData, d => yValue(d)))
                .range([height, 0])
                .nice();
    ...
    chart.displayMode = function(value) {
      if (!arguments.length) {
        return displayMode;
      }
      displayMode = value;
