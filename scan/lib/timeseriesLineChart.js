var timeseriesLineChart = function () {
  ...
  function zoomed() {
    ...
    g.select(".brush").call(brush.move, xScale.range().map(t.invertX, t));
  ...
  chart.getSelectedRangeData = function() {
    // let s = d3.event.selection || xScale.range();
    const s = d3.event.selection;
    if (s) {
      const selectedRangeExtent = s.map(xScale.invert, xScale);
      console.log(selectedRangeExtent);
      const selectedData = chartData.filter((d) => {
        return d.date >= selectedRangeExtent[0] && d.date <= selectedRangeExtent[1];
      });
      return selectedData;
    } 
    return null;
  ...
  chart.rangeSelectionHandler = function (value) {
    if (!arguments.length) {
      return rangeSelectionHandler;
    }
    rangeSelectionHandler = value;
    return chart;
  }
